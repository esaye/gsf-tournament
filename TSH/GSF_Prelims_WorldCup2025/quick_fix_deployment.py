...
def quick_deploy():
    ...
    print("🚀 QUICK FIX: Deploying tournament files to scrabble.ebrimasaye.com/tournament")
    print("=" * 70)
    ...
    try:
        ...
        rsync_command = [
            ...
            '--delete',
            f"{html_dir}/",
            f"ebrimasaye@scrabble.ebrimasaye.com:htdocs/tournament/"
        ]
        
        print(f"Running: {' '.join(rsync_command)}")
        
        result = subprocess.run(rsync_command, capture_output=True, text=True, timeout=30)
    ...
    try:
        ...
        tar_command = [
            'tar', '-czf', '/tmp/tournament_files.tar.gz', 
            '-C', html_dir, '.'
        ]
        ...
        if result.returncode == 0:
            # Upload the tar file
            scp_command = [
                'scp', '/tmp/tournament_files.tar.gz', 
            ...
            if result.returncode == 0:
                ...
                print("📝 Next steps:")
                print("   1. SSH to your server: ssh ebrimasaye@scrabble.ebrimasaye.com")
                print("   2. Extract files: cd htdocs && mkdir -p tournament && cd tournament")
