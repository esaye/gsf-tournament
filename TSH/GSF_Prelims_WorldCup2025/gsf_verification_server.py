...
def log_activity(player_id, action, details):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO activity_log (player_id, action, details) VALUES (?, ?, ?)",
...
def authenticate_player():
    ...
    if player_id in PLAYERS and PLAYERS[player_id]["pin"] == pin:
        log_activity(player_id, "login", "Successful authentication")
        ...
        log_activity(player_id, "login_failed", "Failed authentication attempt")
...
def submit_score():
    ...
    if existing:
        ...
        if submitted_by == player_id:
            conn.close()
            return jsonify(
                {"success": False, "error": "You already submitted this score"}
            )

        # This is opponent verification - check if scores match
        cursor.execute(
            "SELECT player1_score, player2_score, player1_id FROM pending_scores WHERE id = ?",
            (existing_id,),
        )
        ...
        if existing_p1_id == player_id:
            ...
            scores_match = (
                player_score == existing_p1_score
                and opponent_score == existing_p2_score
            )
        else:
            # Current player was player2 in original submission
            scores_match = (
                opponent_score == existing_p1_score
                and player_score == existing_p2_score
            )
        ...
        if scores_match:
            ...
            notify_td_dispute(
                ...
                f"{existing_p1_score}-{existing_p2_score}",
            )
        ...
        # Notify opponent
        notify_opponent(
            opponent_id, player_id, round_num, f"{player_score}-{opponent_score}"
        )
...
def update_tsh_scores(player1_id, player2_id, round_num, score1, score2):
    ...
    try:
        ...
        url = (
            f"{TSH_HOST}/?a=setscores&v={score_string}&d=A&p={p1_tsh_id}&r={round_num}"
        )
        ...
        if response.status_code == 200:
            print(
                f"✅ TSH updated: R{round_num} {player1_id}({score1}) vs {player2_id}({score2})"
            )
            log_activity(
                "system", "tsh_update", f"Updated TSH: R{round_num} {score_string}"
            )
        else:
...
def notify_opponent(opponent_id, submitter_id, round_num, score):
    ...
    print(
        f"📱 NOTIFICATION: {PLAYERS[opponent_id]['name']} - verify R{round_num} score {score} from {PLAYERS[submitter_id]['name']}"
    )
