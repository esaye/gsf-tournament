    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(found_players) >= 6:  # At least 6 players should be visible
                print(f"✅ Division A page accessible with {len(found_players)} players visible")
                return True
            else:
                print(f"⚠️ Division page accessible but only {len(found_players)} players visible")
                return False
        else:
...
def test_photo_files():
    ...
    # Count photo files
    photo_files = [f for f in os.listdir(photo_dir) if f.endswith('.jpg') or f.endswith('.gif')]
...
def test_pairing_generation():
    ...
    try:
        ...
        ], capture_output=True, text=True, cwd='/home/ebrimasaye/TSH/GSF_Prelims_WorldCup2025')
