...
package TSH::ParseArgs::DebugCode;
...
TSH::ParseArgs::DebugCode - handle a command-line argument that should be a debug code

=head1 SYNOPSIS

  my $parser = new TSH::ParseArgs::DebugCode;
...
=item $parserp = new ParserArgs::DebugCode;
...
sub Parse ($$) {
  ...
  if (!defined $value) {
    $line_parser->Error("You need to specify a debug code.");
    return ();
    }
  return $value;
  }
