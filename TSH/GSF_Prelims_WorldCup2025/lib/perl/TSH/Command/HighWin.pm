...
TSH::Command::HighWin - implement the C<tsh> HighWin command

=head1 SYNOPSIS

  my $command = new TSH::Command::HighWin;
...
TSH::Command::HighWin is a subclass of TSH::Command.
...
sub Run ($$@) { 
  ...
  my $setupp = $this->SetupReport(
    'comparator' => sub ($$) { $_[1][0] <=> $_[0][0] }, # big wins first
    'dp' => $dp,
