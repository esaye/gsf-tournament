...
TSH::Command::LowWin - implement the C<tsh> LowWin command

=head1 SYNOPSIS

  my $command = new TSH::Command::LowWin;
...
TSH::Command::LowWin is a subclass of TSH::Command.
...
sub Run ($$@) { 
  ...
  my $setupp = $this->SetupReport(
    'comparator' => sub ($$) { $_[0][0] <=> $_[1][0] }, # small wins first
    'dp' => $dp,
    'max_entries' => $max_entries,
    'selector' => sub ($) { (defined $_[0][0]) && (defined $_[0][1]) && $_[0][0] > $_[0][1] }, # wins only
    'type' => 'lowwin',
    );
  my @types;
