...
TSH::Command::LowLoss - implement the C<tsh> LowLoss command

=head1 SYNOPSIS

  my $command = new TSH::Command::LowLoss;
...
TSH::Command::LowLoss is a subclass of TSH::Command.
...
sub Run ($$@) { 
  ...
  my $setupp = $this->SetupReport(
    'comparator' => sub ($$) { $_[0][0] <=> $_[1][0] }, # small losses first
    'dp' => $dp,
